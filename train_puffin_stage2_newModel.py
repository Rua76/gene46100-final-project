...
motifs = np.load("./newModels/stage1_40000_7.selectmotifs0.95.10motifs.conv.npy")
motifs_deconv = np.load("./newModels/stage1_40000_7.selectmotifs0.95.10motifs.deconv.npy")
inrs = np.load("./newModels/stage1_40000.selectinrs0.95.3inrf.npy")
inrs_deconv = np.load("./newModels/stage1_40000.selectinrs0.95.3inrf.deconv.npy")
...
class EnhancedSimpleNet(nn.Module):
    def __init__(self):
        ...
        # Primary motif detection pathway (large kernel)
        self.conv = nn.Conv1d(4, n_motifs, kernel_size=51, padding=25)
        self.bn_conv = nn.BatchNorm1d(n_motifs * 2)
        
        # Intermediate motif detection (medium kernel)
        self.conv_inr = nn.Conv1d(4, 10, kernel_size=15, padding=7)
        self.bn_conv_inr = nn.BatchNorm1d(20)
        
        # Local feature detection (small kernel)
        self.conv_sim = nn.Conv1d(4, 32, kernel_size=3, padding=1)
    ...
    def forward(self, x, withscaler=True):
        # Process forward and reverse complement for each pathway
        y = torch.cat([self.conv(x), self.conv(x.flip([1, 2])).flip([2])], 1)
        y = self.bn_conv(y)
        y = self.activation(y) 
        ...
        y_sim = self.activation(y_sim)
        
        # Apply scalers if needed
        if withscaler:
...
distilled_net = EnhancedSimpleNet().cpu()
...
for niter in range(100):
    ...
    if niter < 200:
        if validloss < bestloss:
            bestloss = validloss
            torch.save(distilled_net.state_dict(), "./newModels/" + modelstr + ".pth")
    else:
        if validloss < bestloss:
            bestloss = validloss
            torch.save(distilled_net.state_dict(), "./newModels/" + modelstr + ".1000.pth")
